...
const l = [];
l.push(`Garimpo de ${garimpado_em}: **${candidatos.length} candidato(s)** passaram no filtro.`, '');
...
for (const c of candidatos) {
  ...
  l.push(`- **${c.nome}**`);
  l.push(`  ${real(c.preco)} (era ${real(c.preco_original)}, −${c.desconto_percentual}%) · \`${c.meli_id}\``);
  l.push(`  ${c.url_do_produto}`);
}
l.push('', 'Nada disso entrou no site: candidato só vira produto com link de afiliado.');
l.push('Fila completa em `data/candidatos.json`.');
